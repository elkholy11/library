...
@section('title', __('dashboard.edit_author'))

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">@lang('dashboard.edit_author')</h1>
    </div>
            ...
            <form action="{{ route('dashboard.authors.update', $author->id) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('ar_name') is-invalid @enderror" id="ar_name" name="ar_name" value="{{ old('ar_name', $author->ar_name) }}">
                        @error('ar_name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <select id="status" name="status" class="form-select @error('status') is-invalid @enderror" required>
                        <option value="active" @if(old('status', $author->status) == 'active') selected @endif>@lang('dashboard.active')</option>
                        <option value="inactive" @if(old('status', $author->status) == 'inactive') selected @endif>@lang('dashboard.inactive')</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
